...
import (
	...
	"net/url"
	"strings"
	"telegramBot/internal/clients/rss"
...
func (p *Processor) retelling(ctx context.Context, chatID int, callbackID string, text string) (err error) {
	...
	if !ok {
		...
		if err := p.tg.AnswerCallbackQuery(ctx, callbackID, msgImpossibleRetelling); err != nil {
			return err
		}
...
func (p *Processor) getNewsAndSendMessage(ctx context.Context, chatID int, newsFeedInfo storage.News) (err error) {
	...
	case "RSS":
		...
		// slice для удаления shortkey
		shortKeyBuffer := make([]string, 0, len(parsedNewsArr))
		...
		for _, parsedNews := range parsedNewsArr {
			...
			shortKeyBuffer = append(shortKeyBuffer, shortKey)
